...
export default function QuizSection({ topic, onBack, onComplete }: QuizSectionProps) {
  ...
  const loadQuiz = async () => {
    try {
      const response = await fetch(`${backendUrl}/topics/${topic.id}/quiz`)
  ...
  if (quiz.length === 0) {
    return (
      <div className="text-center">
        <h2 className="text-2xl font-bold mb-4">{t.noQuizAvailable}</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <h2 className="text-3xl font-bold text-gray-800">{t.quiz}: {topic.name}</h2>
        <div className="text-sm text-gray-600">
          {t.question} {currentIndex + 1} {t.of} {quiz.length}
        </div>
      </div>
      ...
      <div className="text-center">
        ...
        >
          {currentIndex === quiz.length - 1 ? t.finishQuiz : t.nextQuestion}
        </button>
      </div>
    </div>
  )
